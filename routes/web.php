...
Route::group(['middleware' => ['XSS']], function () {
    ...
    Route::middleware(['auth','set_locale', 'has_permission', 'is_first_login'])->group(function () {
        ...
        Route::controller(App\Http\Controllers\FloorController::class)->group(function () {
            Route::get('/floor', 'index')->name('floor.index');
            Route::get('/floor/create', 'create')->name('floor.create');
            Route::post('/floor', 'store')->name('floor.store');
            Route::get('/floor/{id}/edit', 'edit')->name('floor.edit');
            Route::patch('/floor/{id}', 'update')->name('floor.update');
            Route::delete('/floor/{id}', 'destroy')->name('floor.destroy');
        });

        Route::controller(App\Http\Controllers\ProductionTableController::class)->group(function () {
            Route::get('/production_table', 'index')->name('production_table.index');
            Route::get('/production_table/create', 'create')->name('production_table.create');
            Route::post('/production_table', 'store')->name('production_table.store');
            Route::get('/production_table/{id}/edit', 'edit')->name('production_table.edit');
            Route::patch('/production_table/{id}', 'update')->name('production_table.update');
            Route::delete('/production_table/{id}', 'destroy')->name('production_table.destroy');
        });
